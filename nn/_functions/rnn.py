...
def STLSTMCell(input, hidden, w_ih, w_hhs, w_hht, w_phs, w_pht, b_ih=None, b_hht=None, b_hhs=None, b_phs=None, b_pht=None):

    pi = 3.14159265359
    hidden_s, hidden_t = hidden
    hxj, cxj = hidden_s
    hxt, cxt = hidden_t
    gates = F.linear(input, w_ih, b_ih) + F.linear(hxj, w_hhs, b_hhs) + F.linear(hxt, w_hht, b_hht)

    ingate, forgetgate_s, forgetgate_t, cellgate, outgate = gates.chunk(5, 1)

    ingate = torch.sigmoid(ingate)
    forgetgate_s = torch.sigmoid(forgetgate_s)
    forgetgate_t = torch.sigmoid(forgetgate_t)
    cellgate = torch.tanh(cellgate)
    outgate = torch.sigmoid(outgate)

    p = torch.tanh(F.linear(hxj, w_phs, b_phs) + F.linear(hxt, w_pht, b_pht))
    xd = cellgate
    tau = torch.exp(-torch.pow(p-xd, 2)/2) / pi ** 0.5

    cy = (tau * ingate * cellgate) + ((1-tau) * forgetgate_s * cxj) + ((1-tau) * forgetgate_t * cxt)
    hy = outgate * torch.tanh(cy)
...
def Recurrent(inner, reverse=False):
    def forward(input, hidden, weight, batch_sizes):
        ...
        :param input:
        :param hidden: size should be (scalar(hj-1,t), list(ht-1))
        :param weight:
        :param batch_sizes:
        :return:
        """
        ...
        if inner.__name__ == 'STLSTMCell':
            hidden = (hidden, hidden)
            for i in steps:
                hidden = inner(input[i], hidden, *weight)
                # hack to handle LSTM
                output.append(hidden[0] if isinstance(hidden, tuple) else hidden)
                hidden_o.append(hidden[1])
